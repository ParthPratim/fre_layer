...
def normalize_images():
    for celeb in os.listdir(train_dir):
        celeb_dir = os.path.join(train_dir,celeb)
        for train_img in os.listdir(celeb_dir):
            img_dir = os.path.join(celeb_dir,train_img)
            x = cv2.imread(img_dir)
            y = np.zeros((150,150))
            z = cv2.normalize(x,y,0,255,norm_type=cv2.NORM_MINMAX)
            cv2.imwrite(img_dir,z)
